...
class SwerveDriveBase(startingPose: Pose2d) : SubsystemBase() {
        ...
        when (Constants.mode) {
            Constants.States.REAL -> {
                SwerveDriveConstants.modules.mapIndexed { _, swerveMod ->
                    SwerveModule(
                        SwerveModuleIOKraken(swerveMod),
                        swerveMod.moduleNumber
                    )
                }
            }

            // Although I personally do not know why these are not filled out, I presume it is because this is run during REPLAY mode
            ...
            Constants.States.SIM -> {
                SwerveDriveConstants.modules.mapIndexed { _, swerveMod ->
                    SwerveModule(
                        SwerveModuleIOSim(swerveMod.moduleNumber),
                        swerveMod.moduleNumber
                    )
                }
            }
    ...
    init {
        ...
        field.getObject("Robot").setPoses(
            Pose2d(
        ...
        if (gyroInputs.yawVelocityRadPerSec < Math.PI) {
            ...
            if (camera!!.frame.hasTargets() && DriverStation.isDisabled()) {
                ...
                visionEst?.ifPresent { est ->
                    field2.robotPose = Pose2d(
                        est.estimatedPose.toPose2d().x,
                        est.estimatedPose.toPose2d().y,
                        est.estimatedPose.toPose2d().rotation
                    )
    ...
    fun constructModuleStatesFromChassisSpeeds(
        xSpeedMetersPerSecond: Double,
        ySpeedMetersPerSecond: Double,
        turningSpeedDegreesPerSecond: Double,
        ...
        return kinematics.toSwerveModuleStates(
            if (fieldOriented) {
                ChassisSpeeds.fromFieldRelativeSpeeds(
                    ...
                    turningSpeedDegreesPerSecond.rotation2dFromDeg().radians,
    ...
    fun constructTeleopModuleStatesFromChassisSpeeds(
        xSpeedMetersPerSecond: Double,
        ySpeedMetersPerSecond: Double,
        turningSpeedDegreesPerSecond: Double,
        fieldOriented: Boolean
        ...
        return kinematics.toSwerveModuleStates(
            if (fieldOriented) {
                ChassisSpeeds.fromFieldRelativeSpeeds(
                    ...
                    turningSpeedDegreesPerSecond.rotation2dFromDeg().radians,
                    if (red) {
                        (poseEstimator.estimatedPosition.rotation + 180.0.rotation2dFromDeg())
                    } else {
                        (poseEstimator.estimatedPosition.rotation)
                    }
                )
